...
use mongodb::{
    bson::Document,
    error::Result,
    options::{ClientOptions, FindOptions, Tls, TlsOptions},
...
impl MongoDB {
    ...
    pub async fn delete_many<T: Serialize>(
        &self,
        collection_name: &str,
        query: Document,
    ...
    pub async fn delete_one<T: Serialize>(
        &self,
        collection_name: &str,
        query: Document,
    ) -> Result<mongodb::results::DeleteResult> {
    ...
    pub async fn drop<T: Serialize>(&self, collection_name: &str) -> Result<()> {
        let c = &self.get_collection::<T>(collection_name);
        c.drop(None).await
    }

    pub async fn count_documents<T: Serialize>(
        &self,
        collection_name: &str,
        filter: Document,
    ) -> Result<u64> {
...
fn parse_url(cfg: &MongoCfg) -> String {
    match (&cfg.user_name, &cfg.password) {
        (Some(user_name), Some(password)) => format!(
            "mongodb://{}:{}@{}:{}",
            user_name, password, cfg.host, cfg.port
        ),
        _ => format!("mongodb://{}:{}", cfg.host, cfg.port),
    }
}
